...
function seriesCandidatesForEvent(event){
  ...
  // Bereits bestehende Trainings ohne Serien-ID:
  // gleiche Art, gleiche Uhrzeit und gleiche Bezeichnung gelten als Serie.
...
function openEventSeriesEditor(eventId){
  ...
  openModal(`
    ...
    <div class="stack">
      ...
      ${hasSeries?`
        <p class="muted">
          Bei «zukünftige Trainings» werden alle Termine ab ${fmtDate(event.date)} angepasst.
          Das Datum selbst wird nur geändert, wenn «Nur dieses Training» gewählt ist.
        </p>
      `:''}

      <div class="row">
        <button class="btn primary" onclick="saveEventSeriesEdit('${eventId}')">Änderungen speichern</button>
        <button class="btn danger" onclick="deleteEventSeries('${eventId}')">Training/Serie löschen</button>
      </div>
    </div>
  `);
}
...
function saveEventSeriesEdit(eventId){
  ...
  const seriesId=event.seriesId||('series_'+event.id);

  for(const item of targets){
    item.time=newTime;
    item.title=newTitle;

    // Das Datum darf nur bei einem einzelnen Termin direkt geändert werden.
    if(scope==='single'){
      item.date=newDate;
    }
...
const POWERPLAY_POSITIONS=[
  {key:'P1',label:'Position 1'},
  {key:'P2',label:'Position 2'},
  {key:'P3',label:'Position 3'},
  {key:'P4',label:'Position 4'},
  {key:'P5',label:'Position 5'}
];
const BOXPLAY_POSITIONS=[
  {key:'P1',label:'Position 1'},
  {key:'P2',label:'Position 2'},
  {key:'P3',label:'Position 3'},
  {key:'P4',label:'Position 4'}
];
const SPECIAL_TEAM_BLOCKS=[
  {key:'PP1',label:'PP1',positions:POWERPLAY_POSITIONS},
  {key:'PP2',label:'PP2',positions:POWERPLAY_POSITIONS},
  {key:'BP1',label:'BP1',positions:BOXPLAY_POSITIONS},
  {key:'BP2',label:'BP2',positions:BOXPLAY_POSITIONS},
  {key:'BP3',label:'BP3',positions:BOXPLAY_POSITIONS}
];
function ensureLineup(eventId){
  ...
  data.lineups[eventId].specialTeams ||= {};
  for(const block of SPECIAL_TEAM_BLOCKS){
    data.lineups[eventId].specialTeams[block.key] ||= {};
    for(const pos of block.positions){
      if(!(pos.key in data.lineups[eventId].specialTeams[block.key])) data.lineups[eventId].specialTeams[block.key][pos.key]=null;
    }
  }
}
...
function makeSpecialTeamSlot(eventId,blockKey,posKey,label,pid){
  const slot=document.createElement('div');
  slot.className='slot special-team-slot';
  const player=data.players.find(p=>p.id===pid);
  slot.innerHTML=`<div class="slot-label">${label}</div>${player?`<div class="assigned">${player.name}</div><button class="remove" onclick="removeFromSpecialTeam('${eventId}','${blockKey}','${posKey}')">Entfernen</button>`:'<div class="muted">Spieler hierher ziehen</div>'}`;
  slot.addEventListener('dragover',ev=>{ev.preventDefault();slot.classList.add('dragover')});
  slot.addEventListener('dragleave',()=>slot.classList.remove('dragover'));
  slot.addEventListener('drop',ev=>{
    ev.preventDefault();slot.classList.remove('dragover');
    const pid=ev.dataTransfer.getData('text/plain');
    assignToSpecialTeam(eventId,blockKey,posKey,pid);
  });
  return slot;
}
function assignToSpecialTeam(eventId,blockKey,posKey,pid){
  ensureLineup(eventId);
  const block=data.lineups[eventId].specialTeams[blockKey];
  for(const key of Object.keys(block)) if(block[key]===pid) block[key]=null;
  block[posKey]=pid;
  save();
}
function removeFromSpecialTeam(eventId,blockKey,posKey){
  ensureLineup(eventId);
  data.lineups[eventId].specialTeams[blockKey][posKey]=null;
  save();
}
function renderSpecialTeams(eventId,eligiblePlayers){
  const lineup=data.lineups[eventId];
  const section=document.createElement('div');
  section.className='special-teams-section';
  section.innerHTML='<h2>Special Teams</h2><div class="muted">Spieler können zusätzlich zur normalen Aufstellung in Powerplay und Boxplay eingesetzt werden.</div>';

  const pool=document.createElement('div');
  pool.className='player-pool special-teams-player-pool';
  for(const p of eligiblePlayers){
    const item=document.createElement('div');
    item.className='drag-player';
    item.textContent=p.name;
    item.draggable=true;
    item.dataset.playerId=p.id;
    item.addEventListener('dragstart',ev=>ev.dataTransfer.setData('text/plain',p.id));
    pool.appendChild(item);
  }
  section.appendChild(pool);

  const groups=document.createElement('div');
  groups.className='special-teams-groups';
  for(const block of SPECIAL_TEAM_BLOCKS){
    const card=document.createElement('div');
    card.className='line-row special-team-block';
    card.innerHTML=`<h3>${block.label}</h3>`;
    const slots=document.createElement('div');
    slots.className=block.key.startsWith('PP')?'forward-row special-team-row pp-row':'forward-row special-team-row bp-row';
    for(const pos of block.positions){
      slots.appendChild(makeSpecialTeamSlot(eventId,block.key,pos.key,pos.label,lineup.specialTeams[block.key][pos.key]));
    }
...
function renderLineup(eventId){
  ...
  board.appendChild(renderSpecialTeams(eventId,eligiblePlayers));
}
...
function lineupReport(eventId){
  ...
  out+='</table><h3>Special Teams</h3>';
  for(const block of SPECIAL_TEAM_BLOCKS){
    out+=`<h4>${block.label}</h4><table><tr>${block.positions.map(p=>`<th>${p.label}</th>`).join('')}</tr><tr>${block.positions.map(p=>`<td>${pname(data.lineups[eventId].specialTeams[block.key][p.key])}</td>`).join('')}</tr></table>`;
  }
  return out;
}
...
// Special Teams – Zusatzlayout
(()=>{
  if(document.getElementById('specialTeamsStyles'))return;
  const style=document.createElement('style');
  style.id='specialTeamsStyles';
  style.textContent=`
    .special-teams-section{margin-top:28px;padding-top:22px;border-top:2px solid #dfe7e2}
    .special-teams-section h2{margin:0 0 6px}
    .special-teams-player-pool{margin:14px 0 18px;display:flex;flex-wrap:wrap;gap:8px}
    .special-teams-groups{display:grid;gap:16px}
    .special-team-block{margin:0}
    .special-team-row{display:grid!important;gap:10px}
    .special-team-row.pp-row{grid-template-columns:repeat(5,minmax(0,1fr))}
    .special-team-row.bp-row{grid-template-columns:repeat(4,minmax(0,1fr))}
    .special-team-slot{min-width:0}
    @media(max-width:800px){
      .special-team-row.pp-row,.special-team-row.bp-row{grid-template-columns:1fr 1fr}
      .special-team-row .slot:last-child:nth-child(odd){grid-column:1/-1}
    }
  `;
